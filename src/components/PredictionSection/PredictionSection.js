...
function PredictionSection() {
	...
	const submitResponse = (e) => {
		...
		const dataToSend = {
			Age: age,       // age has to be with in 1 to 120
			Gender: gender, // 0 means Female and 1 means Male
			AirPollution: (airPollution/10) * 8,
			Alcoholuse: (alcoholUse/10) * 8,
			DustAllergy: (dustAllergy/10)*8,
			OccuPationalHazards: (occupationalHazards/10)*8,
			GeneticRisk: (geneticRisk/10)*7,
			chronicLungDisease: (chronicLungDisease/10)*7,
			BalancedDiet: (balancedDiet/10)*7,	
			Obesity: (obesity/10)*7,
			Smoking: (smoking/10)*8,
			PassiveSmoker: (passiveSmoker/10)*8,
			ChestPain: (chestPain/10)*9,
			CoughingofBlood: (coughingOfBlood/10)*9,
			Fatigue: (fatigue/10)*9,
			WeightLoss: (weightLoss/10)*8,
			ShortnessofBreath: (shortnessOfBreath/10)*9,
			Wheezing: (wheezing/10)*8,
			SwallowingDifficulty: (swallowingDifficulty/10)*8,
			ClubbingofFingerNails: (clubbingOfFingerNails/10)*9,
			FrequentCold: (frequentCold/10)*7,
			DryCough: (dryCough/10)*7,
			Snoring: (snoring/10)*7,
		};
		...
		fetch('https://lung-cancer-prognosis-backend.herokuapp.com/predict', requestOptions)
